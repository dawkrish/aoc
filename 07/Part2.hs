...
main = do
  ...
  let hands = map (\x -> Hand (head . words $ x) (jokerVariant (head . words $ x)) (getHandRank (jokerVariant . head . words $ x)) (getNumber . last . words $ x)) sentences
...
-- WE CAN EXCLUDE 'jokerV'. It is just for demonstration purpose
data Hand = Hand {val :: String, jokerV :: String, rank :: Int, bid :: Int}
  deriving (Show)
...
instance Ord Hand where
  compare (Hand val1 j1 r1 _) (Hand val2 j2 r2 _)
    | r1 > r2 = GT
    | r2 > r1 = LT
...
jokerVariant :: String -> String
jokerVariant "JJJJJ" = "JJJJJ"
jokerVariant xs = [if x == 'J' then foo else x | x <- xs]
  where
    remJs = filter (/= 'J') xs
    foo = fst (last (sortOn snd (frequency remJs)))
